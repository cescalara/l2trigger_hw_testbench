...
typedef struct {
    ...
    XL2_trigger_uio_map maps[ MAX_UIO_MAPS ];
} XL2_trigger_uio_info;

/***************** Variable Definitions **************************************/
static XL2_trigger_uio_info uio_info;
...
static int uio_info_read_name(XL2_trigger_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XL2_trigger_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XL2_trigger_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XL2_trigger_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XL2_trigger_Initialize(XL2_trigger *InstancePtr, const char* InstanceName) {
	XL2_trigger_uio_info *InfoPtr = &uio_info;
